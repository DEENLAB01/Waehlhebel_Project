...
# -*- coding: latin-1 -*-
#
# File    : CRC_Fehler_ORU_OTAMC_D_01.py
# Title   : CRC_Fehler_ORU_OTAMC_D_01
# Task    : ORU_OTAMC_D Fehlererkennung für CRC-Fehler im Fehlerspeicher
#
...
# 1.1  | 19.05.2022  | Mohammed | 3 OP-PowerCycle
# 1.2  | 14.07.2022  | Mohammed | Rework
# 1.3  | 27.07.2022  | Mohammed  | Added waiting time during ECU_Sleep
#******************************************************************************
...
try:
    ...
    active_dtcs = [(0xE0010F, 0x27)]
    passive_dtcs = [(0xE0010F, 0x24)]
    confirme_dtcs = [(0xE0010F, 0x2F)]
    curr_value = func_nm.low_current()
    # set Testcase ID #########################################################
    testresult.setTestcaseId("TestSpec_191")
    ...
    testresult.append(["[.] Lese Fehlerspeicher (muss leer sein)", ""])
    testresult.append(canape_diag.checkEventMemoryEmpty(ticket_id='FehlerId:EGA-PRM-252'))
    testresult.append(["[-0]", ""])

    # test step 5-5.2
    testresult.append(["[.] Sende zweiten CRC-Fehler für OTAMC_D_01::OTAMC_D_01_CRC ", ""])
    sec = 0.320
    timeout = sec + t()
    while timeout > t():
        hil.OTAMC_D_01__OTAMC_D_01_CRC__value.set(0)
    ...
    # test step 7-7.2
    testresult.append(["[.] Sende keinen CRC-Fehler mehr für ORU_Control_D_01::ORU_Control_D_01_CRC", ""])
    testresult.append(["[+] Warte 1000 ms (2x tMSG_CYCLE )+ 100 (Toleranz)", ""])
    time.sleep(1.1)

    testresult.append(["[.] Lese Fehlerspeicher (0xE0010F DTC aktiv)", ""])
    testresult.append(canape_diag.checkEventMemory(active_dtcs))
    testresult.append(["[-0]", ""])
    ...
    testresult.append(["[.] Sende zwei OTAMC_D_01-CRC Fehler hintereinander", ""])
    sec = .640
    timeout = sec + t()
    while timeout > t():
        hil.OTAMC_D_01__OTAMC_D_01_CRC__value.set(0)
