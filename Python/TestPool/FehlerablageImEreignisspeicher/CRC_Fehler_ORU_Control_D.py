...
# 1.1  | 13.07.2022  | Mohammed | 3 OP-PowerCycle
...
try:
    ...
    passive_dtcs = [(0xE0010E, 0x24)]
    confirme_dtcs = [(0xE0010E, 0x2F)]
    ...
    testresult.append(["[.] Lese Fehlerspeicher (muss leer sein)", ""])
    testresult.append(canape_diag.checkEventMemoryEmpty(ticket_id='FehlerId:EGA-PRM-251'))
    testresult.append(["[-0]", ""])

    # test step 5-5.2
    testresult.append(["[.] Sende zweiten CRC-Fehler für ORU_Control_D_01::ORU_Control_D_01_CRC", ""])
    sec = 0.320
    timeout = sec + t()
    while timeout > t():
        hil.ORU_Control_D_01__ORU_Control_D_01_CRC__value.set(0)
    ...
    testresult.append(["[+] Warte 1100 ms (2x tMSG_CYCLE + Toleranz)", ""])
    time.sleep(1.1)
    ...
    testresult.append(["[.] Sende zwei ORU_Control_D-CRC Fehler hintereinander", ""])
    sec = .640
    timeout = sec + t()
    while timeout > t():
        hil.ORU_Control_D_01__ORU_Control_D_01_CRC__value.set(0)
