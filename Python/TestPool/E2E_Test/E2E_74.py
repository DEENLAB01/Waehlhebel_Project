...
# -*- coding: latin1 -*-
# File    : E2E_74.py
# Title   : ORU_Control_D_Timeout_Normal
# Task    : Erstmalige Timeout-Fehlererkennung ORU_Control_D_01 während "Normal"  und Heilung
#
...
def checkProgrammingPrecondition(exp_content):
    ...
    """
    # test step
    # testresult.append(["\xa0 Programmiervorbedingungen prüfen: 0x3101 + {}"
...
try:
    ...
    # set Testcase ID #########################################################
    testresult.setTestcaseId("E2E_74")
    ...
    # test step 1
    testresult.append(["[.] Schaltle Kl30 und Kl15 und RBS ein und warte 2500ms", ""])
    hil.cl15_on__.set(1)
    ...
    # test step 2
    testresult.append(["[-] Setze ORU_Control_D_01 zyklus zeit auf  0ms", ""])
    hil.ORU_Control_D_01__period.set(0)

    # test step 3
    testresult.append(["[.] Warte  2880ms. (Qualifikation Zeit für Timeout DTC eintrag = n-q+ 1= 10-2+1= 9 fehlende Botschaft im FIFO )", ""])
    time.sleep(2.880 + 0.220)
    ...
    # test step 5
    testresult.append(["[.] Lese Fehlerspeicher (0xE00108 DTC aktiv)", ""])
    testresult.append(canape_diag.checkEventMemory(aktiv_dtc))

    # test step 6
    testresult.append(["[.] Setze ORU_Control_D zyklus zeit auf 320ms und warte 2560ms+ 160ms Toleranze (3 + n/2 gültige signal im FIFO)", ""])
    hil.ORU_Control_D_01__period.set(320)
    time.sleep(2.560 + 0.160)
