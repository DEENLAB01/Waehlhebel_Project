...
# -*- coding: latin1 -*-
# File    : E2E_89.py
# Title   : ORU_Control_A_CRC_BZ_Error
...
try:
    ...
    msg = hil.ORU_Control_A_01__ORU_Control_A_01_CRC__value

    aktiv_dtc = [(0xE00107, 0x27)]
    passiv_dtc = [(0xE00107, 0x26)]

    aktiv_dtc_bz = [(0xE0010A, 0x27)]
    passiv_dtc_bz = [(0xE0010A, 0x26)]

    # set Testcase ID #########################################################
    testresult.setTestcaseId("E2E_89")
    ...
    # test step 2
    testresult.append(["[-] Halte RU_Control_A_01_BZ.", ""])
    hil.ORU_Control_A_01__ORU_Control_A_01_BZ__switch.set(1)

    # test step 3
    testresult.append(["[.] Warte 2 Zyklen lang (2x500=1000ms)", ""])
    time.sleep(1)

    # test step 4
    testresult.append(["[.]  Sende einmal ORU_Cotrol_A_01_CRC Fehler (Wechsel nach Safe State)", ""])
    sec = 0.490
    timeout = sec + t()
    CRC_value = hil.ORU_Control_A_01__ORU_Control_A_01_CRC__value.get()

    hil.ORU_Control_A_01__ORU_Control_A_01_CRC__value.set(0)
    while timeout > t():
        hil.ORU_Control_A_01__ORU_Control_A_01_CRC__value.set(0)

    hil.ORU_Control_A_01__ORU_Control_A_01_CRC__value.set(CRC_value)
    ...
    # test step 10
    testresult.append(["[.] Setze  ORU_Control_A_01_BZ  wider fort und warte 4000ms + 400ms Toleranze (3+ n/2 gültige signal im FIFO)",""])
    hil.ORU_Control_A_01__ORU_Control_A_01_BZ__switch.set(0)
    time.sleep(4.000 + 0.400)
