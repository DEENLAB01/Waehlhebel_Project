...
# -*- coding: latin1 -*-
# File    : E2E_58.py
# Title   : ORU_Control_A_BZ_Error_Diag_Suppressed
# Task    : Erstmalige Fehlererkennung ORU_Control_A_01_BZ während "Initialized" bei ausgeblendeter Diagnose.
#
...
# ------------------------------------------------------------------------------
# 1.0  | 27.07.2022 | Devangbhai   | initial
# 1.1  | 19.08.2022 | Devangbhai   | Added correct Precondition
# 1.2  | 26.10.2022 | Devangbhai   | Added 200ms in test step 2.1 for tDiagStart, Change the wait time in test step 4
...
try:
    ...
    # set Testcase ID #########################################################
    testresult.setTestcaseId("E2E_58")
    ...
    # test step 1 and 2
    testresult.append(["[.] Schalte Kl30 ein, setze ORU_Control_A_01 Zykluszeit auf 0.", ""])
    ...
    hil.cl15_on__.set(1)
    func_nm.hil_ecu_e2e(allstate=1, sisft=1, otamc=1, oruA=0, ourD=1)
    ...
    # test step 3
    testresult.append(["[-] Setze ORU_Control_A_01 zyklus zeit auf 500ms und sende permament ORU_Control_A_01_BZ Fehler (Wechsel nach Safe State)", ""])
    hil.ORU_Control_A_01__ORU_Control_A_01_BZ__switch.set(1)
    hil.ORU_Control_A_01__period.setState("an")

    # test step 4
    testresult.append(["[.] Warte 4500ms (n -q +1 n.i.O Botschaft im FIFO =10-2 + 1= 9*500ms = 4500ms) + 500ms (1st ist gültige Botschaft)+ 250ms(Jitter)", ""])
    time.sleep(4.5 + 0.500 + 0.250)
