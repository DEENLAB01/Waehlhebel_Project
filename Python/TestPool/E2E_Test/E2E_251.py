...
try:
    ...
    OTAMCD_CRC= crc_dict['OTAMC_D_01']
    exp_wrong_prec = [0xA7]

    aktiv_dtc = [(0xE0010C, 0x27)]
    passiv_dtc = [(0xE0010C, 0x26)]

    # set Testcase ID #########################################################
    testresult.setTestcaseId("E2E_251")
    ...
    while True:
        BZ_value = hil.OTAMC_D_01__OTAMC_D_01_BZ__value.get()
        if BZ_value == 0:
    ...
    # test step 3
    testresult.append(["[.]  Überschreibe OTAMC_D_BZ 4x (n/3+1) mit je einer S-PDU mit um (BSZ_DeltaMaxInit+3 = 5)"
    ...
    for i in bz_:
        if i== bz_[-1]:
            hil.OTAMC_D_01__OTAMC_D_01_CRC__value.set(OTAMCD_CRC['%s' % i])
            hil.OTAMC_D_01__OTAMC_D_01_BZ__value.set(i)
            print i, "THIS VALUE IS SET", OTAMCD_CRC['%s' % i]
