# ******************************************************************************
# -*- coding: latin-1 -*-
# File    : Verify_ORU_Control_D_read_DUT.py
# Title   : Verify_ORU_Control_D_read_DUT
...
try:
    ...
    # set Testcase ID #########################################################
    testresult.setTestcaseId("TestSpec_359")
    ...
    # test step 8
    testresult.append(["[.] Lese aktuelle Diagnose Session aus", ""])
    testresult.extend(canape_diag.checkDiagSession('default', ticket_id='FehlerId:EGA-PRM-233'))
    ...
    testresult.append(canape_diag.checkPositiveResponse(response, request, ticket_id='FehlerId:EGA-PRM-233'))

    # test step 9.2
    testresult.append(["[.] ]Inhalt der Response überprüfen", ""])
    exp_response = [0x62] + diag_ident['identifier'] + diag_ident['expected_response']
    testresult.append(canape_diag.checkResponse(response, exp_response, ticket_id='FehlerId:EGA-PRM-233'))
    testresult.append(["[-0]", ""])

    # test step 10
    testresult.append(["[.] Wechsel in Default Session: 0x1001", ""])
    testresult.extend(canape_diag.changeAndCheckDiagSession('default',ticket_id='FehlerId:EGA-PRM-233'))
    ...
    # test step 12
    testresult.append(["[.] Wechsel in Extended Session: 0x1003", ""])
    testresult.extend(canape_diag.changeAndCheckDiagSession('extended', ticket_id='FehlerId:EGA-PRM-233'))
    ...
    # test step 14
    testresult.append(["[.] Setze ORU_Control_D_01.OnlineRemoteUpdateControlD auf 6 (PENDING_NOT_READY)", ""])
    hil.ORU_Control_D_01__OnlineRemoteUpdateControlD__value.set(6)
